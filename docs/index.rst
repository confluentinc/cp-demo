...
#. From the ``cp-demo`` directory, start the entire example by running a single command that generates the keys and certificates, brings up the Docker containers, and configures and validates the environment. This will take approximately 7 minutes to complete.
...
There is an OpenLDAP server running in the example, and each Kafka broker in the example is configured with MDS and can talk to LDAP so that it can authenticate clients and |cp| services and clients.
