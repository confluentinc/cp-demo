...
users=(kafka1 kafka2 client schemaregistry restproxy connect connectorSA controlcenter ksqlDBUser appSA badapp clientListen zookeeper mds)
...
# we no longer generating a cert for ksqlDB Server - it shares a cert with Control Center
cp kafka.controlcenter.keystore.jks kafka.ksqlDBserver.keystore.jks
cp kafka.controlcenter.truststore.jks kafka.ksqlDBserver.truststore.jks
