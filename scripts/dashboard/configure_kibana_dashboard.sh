...
curl -X PUT -H "kbn-version: 5.5.2" -d '{"title":"wikipediabot","notExpandable":true, "timeFieldName": "TIMESTAMP"}' http://localhost:5601/es_admin/.kibana/index-pattern/wikipediabot/_create
...
curl -X POST -H "kbn-version: 5.5.2" -H "Content-Type: application/json;charset=UTF-8" -d '{"value":"TIMESTAMP"}' http://localhost:5601/api/kibana/settings/timelion:es.timefield
