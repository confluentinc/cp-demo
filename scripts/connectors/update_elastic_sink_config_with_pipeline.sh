...
DATA=$( cat << EOF
{
  ...
  "schema.ignore": true,
  "elasticsearch.index.param.pipeline": "wikipediabot-createdat-monthlyindex"

}
EOF
)

docker-compose exec connect curl -X PUT -H "${HEADER}" --data "${DATA}" --cert /etc/kafka/secrets/connect.certificate.pem --key /etc/kafka/secrets/connect.key --tlsv1.2 --cacert /etc/kafka/secrets/snakeoil-ca-1.crt -u connectorSubmitter:connectorSubmitter https://connect:8083/connectors/elasticsearch-ksqldb/config
