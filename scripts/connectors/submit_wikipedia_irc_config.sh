...
DATA=$( cat << EOF
{
  ...
  "config": {
    ...
    "value.converter.basic.auth.user.info": "connectSA:connectSA",
    "producer.override.sasl.jaas.config": "org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required username=\"connectSA\" password=\"connectSA\" metadataServerUrls=\"http://kafka1:8090\";",
    "tasks.max": "1"
  }
}
EOF
...
docker-compose exec connect curl -X POST -H "${HEADER}" --data "${DATA}" --cert /etc/kafka/secrets/connect.certificate.pem --key /etc/kafka/secrets/connect.key --tlsv1.2 --cacert /etc/kafka/secrets/snakeoil-ca-1.crt -u connectSA:connectSA https://connect:8083/connectors
